...
const SubSegmentEditor: React.FC<SubSegmentEditorProps> = ({
  ...
  const addSubsegment = () => {
    ...
    if (lastEnd >= segmentExample.length) {
      console.log('Segment bereits vollständig abgedeckt');
      return;
    }
  ...
  const validateAndSave = () => {
    ...
    for (let i = 0; i < subsegments.length; i++) {
      ...
      if (!sub.name.trim()) {
        console.error(`Sub-Segment ${i + 1}: Name fehlt`);
        return;
      }
      
      if (sub.start >= sub.end) {
        console.error(`Sub-Segment ${i + 1}: Start muss kleiner als End sein`);
        return;
      }
      
      if (sub.start < 0 || sub.end > segmentExample.length) {
        console.error(`Sub-Segment ${i + 1}: Position außerhalb des Segments`);
        return;
      }
      ...
      for (let j = i + 1; j < subsegments.length; j++) {
        ...
        if (!(sub.end <= other.start || other.end <= sub.start)) {
          console.error(`Sub-Segmente ${i + 1} und ${j + 1} überlappen sich`);
          return;
        }
      }
    }
